...
async function run() {
    ...
    try {
        ...
        if (filters.length > 1) {
            await filters[1].click(); // Open "Bác sĩ" select
            await driver.sleep(500);
            const options = await driver.findElements(By.xpath("//div[@role='option']"));
            if (options.length > 1) {
                await options[1].click(); // Select the first doctor option
            }
            await driver.sleep(1000);
        }
    
        
        console.log("AT-217 Passed: filter_template_doctor");
    } catch (err) { 
        console.error("AT-217 Failed:", err.message); 
    } finally { 
        await driver.quit(); 
    }
}
