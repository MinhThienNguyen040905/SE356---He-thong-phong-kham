...
async function run() {
    ...
    try {
        ...
        if (filters.length > 2) {
            await filters[2].click(); // Open "Ca trực" select
            await driver.sleep(500);
            const options = await driver.findElements(By.xpath("//div[@role='option']"));
            if (options.length > 1) {
                await options[1].click(); // Select the first shift option
            }
            await driver.sleep(1000);
        }
    
        
        console.log("AT-218 Passed: filter_template_shift");
    } catch (err) { 
        console.error("AT-218 Failed:", err.message); 
    } finally { 
        await driver.quit(); 
    }
}
