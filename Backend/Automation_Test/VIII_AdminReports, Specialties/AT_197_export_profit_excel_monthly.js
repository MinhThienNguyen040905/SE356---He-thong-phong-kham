...
async function run() {
    ...
    try {
        ...
        const tabBtn = await driver.wait(until.elementLocated(By.xpath(`//button[@role='tab' and contains(., '${tabMap['profit']}')]`)), 10000);
        await tabBtn.click();
        ...
        console.log("AT-197 Passed: export_profit_excel_monthly");
    } catch (err) { 
        console.error("AT-197 Failed:", err.message); 
    } finally { 
        await driver.quit(); 
    }
}
