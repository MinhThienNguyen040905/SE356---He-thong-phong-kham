...
async function run() {
    ...
    try {
        ...
        await driver.wait(until.elementLocated(By.id("create-name")), 5000); // leave name empty
        ...
        await driver.wait(until.elementLocated(By.xpath("//div[contains(text(), 'Vui lòng nhập tên chuyên khoa')]")), 5000);
        console.log("AT-158 Passed: Add specialty fail when name is empty");
    } catch (err) { 
        console.error("AT-158 Failed:", err.message); 
    } finally { 
        await driver.quit(); 
    }
}
