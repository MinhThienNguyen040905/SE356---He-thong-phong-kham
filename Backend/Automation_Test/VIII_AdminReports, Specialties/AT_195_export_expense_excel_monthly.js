...
async function run() {
    ...
    try {
        ...
        const tabBtn = await driver.wait(until.elementLocated(By.xpath(`//button[@role='tab' and contains(., '${tabMap['expense']}')]`)), 10000);
        await tabBtn.click();
        ...
        const exportBtns = await driver.findElements(By.xpath("//button[contains(., 'Excel')]"));
        ...
        await driver.wait(until.elementLocated(By.xpath("//*[contains(translate(text(), 'abcdefghijklmnopqrstuvwxyz', 'ABCDEFGHIJKLMNOPQRSTUVWXYZ'), 'EXCEL')]")), 10000);
    
        
        console.log("AT-195 Passed: export_expense_excel_monthly");
    } catch (err) { 
        console.error("AT-195 Failed:", err.message); 
    } finally { 
        await driver.quit(); 
    }
}
