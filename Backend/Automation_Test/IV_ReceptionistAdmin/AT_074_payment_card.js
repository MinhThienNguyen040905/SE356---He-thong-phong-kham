...
async function run() {
    ...
    try {
        ...
        let selectTrigger = await driver.findElement(By.xpath("//button[span[contains(text(), 'Tiền mặt')]]"));
        await selectTrigger.click();
        
        let bankOption = await driver.wait(until.elementLocated(By.xpath("//div[@role='option' and contains(., 'Chuyển khoản ngân hàng')]")), 5000);
        ...
        console.log("AT-074 Passed: Thanh toán chuyển khoản/thẻ thành công.");
    } catch (err) { 
        console.error("AT-074 Failed:", err.message); 
    } finally { 
        await driver.quit(); 
    }
}
