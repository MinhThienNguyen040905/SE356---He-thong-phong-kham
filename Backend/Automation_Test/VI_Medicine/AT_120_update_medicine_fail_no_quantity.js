...
async function run() {
    ...
    try {
        ...
        let quantityField = await driver.findElement(By.id("quantity"));
        let value = await quantityField.getAttribute("value");
        for (let i = 0; i < value.length; i++) {
            await quantityField.sendKeys("\uE003"); 
        }
        ...
        console.log("AT-126 Passed: Cập nhật thất bại khi trống số lượng.");
    } catch (err) {
        console.error("AT-126 Failed:", err.message);
    } finally {
        await driver.quit();
    }
}
